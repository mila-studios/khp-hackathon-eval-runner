...
# Usage: scripts/evaluate.sh <path_to_predictions.csv> <path_to_metrics.csv>
#
# Reads the team's predictions CSV and writes evaluation metrics (precision,
# recall, F1, latency) to the CSV path provided as the second argument.

set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: scripts/evaluate.sh <path_to_predictions.csv> <path_to_metrics.csv>" >&2
  exit 1
fi
...
if [[ -f "$OUTPUT_DIR/metrics.csv" ]]; then
  mv "$OUTPUT_DIR/metrics.csv" "$EVAL_METRICS_CSV" 2>/dev/null || true
